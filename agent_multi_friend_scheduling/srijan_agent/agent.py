...
def generate_srijan_calendar() -> dict[str, list[str]]:
    """Generates a random calendar for Srijan for the next 7 days."""
    calendar = {}
    today = date.today()
    ...
    print("Srijan's calendar:", calendar)

    return calendar


SRIJAN_CALENDAR = generate_srijan_calendar()


def get_availability(start_date: str, end_date: str) -> str:
    """
    Checks Srijan's availability for a given date range.
    ...
    Returns:
        A string listing Srijan's available times for that date range.
    """
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = SRIJAN_CALENDAR.get(date_str, [])
            if available_slots:
                availability = f"On {date_str}, Srijan is available at: {', '.join(available_slots)}."
                results.append(availability)
            else:
                results.append(f"Srijan is not available on {date_str}.")
